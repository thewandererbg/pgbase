...
func TestNewBaseApp(t *testing.T) {
	const testDataDir = "/tmp/pb_base_app_test_data_dir/"
	defer os.RemoveAll(testDataDir)

	dataDB, auxDB, db := generateRandomDB(t)
	defer dropRandomDB(t, db, dataDB, auxDB)
...
func generateRandomDB(t *testing.T) (string, string, *dbx.DB) {
	...
	_, err = db.NewQuery("create database " + dataDB).Execute()
	if err != nil {
		t.Fatalf("failed to create dataDB: %v", err)
	}
	_, err = db.NewQuery("create database " + auxDB).Execute()
	if err != nil {
		t.Fatalf("failed to create auxDB: %v", err)
	}
	return dataDB, auxDB, db
}

func dropRandomDB(t *testing.T, db *dbx.DB, dataDB, auxDB string) {
	_, err := db.NewQuery("drop database " + dataDB).Execute()
	if err != nil {
		t.Fatalf("failed to drop dataDB: %v", err)
	}
	_, err = db.NewQuery("drop database " + auxDB).Execute()
	if err != nil {
		t.Fatalf("failed to drop auxDB: %v", err)
	}
	db.Close()
}
