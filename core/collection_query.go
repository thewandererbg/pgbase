...
func (app *BaseApp) FindCollectionByNameOrId(nameOrId string) (*Collection, error) {
	...
	err := app.CollectionQuery().
		// AndWhere(dbx.NewExp("[[id]]={:id} OR LOWER(LOWER(name))={:name}", dbx.Params{
...
func (app *BaseApp) IsCollectionNameUnique(name string, excludeIds ...string) bool {
	...
	query := app.CollectionQuery().
		Select("count(*)").
		AndWhere(dbx.NewExp("LOWER(LOWER(name))={:name}", dbx.Params{"name": strings.ToLower(name)})).
		Limit(1)
...
// (see https://github.com/thewandererbg/pgbase/issues/3110).
