...
// Postgresql will return the first record that matches the filter in a random order
func TestFindFirstRecordByFilter(t *testing.T) {
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			_, err := app.FindFirstRecordByFilter(s.collectionIdOrName, s.filter, s.params...)
