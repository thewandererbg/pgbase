...
func normalizeSingleVsMultipleFieldChanges(app App, newCollection *Collection, oldCollection *Collection) error {
	...
	return app.RunInTransaction(func(txApp App) error {
		for _, newField := range newCollection.Fields {
			...
			views := []struct {
				Name       string `db:"viewname"`
				Definition string `db:"definition"`
				SchemaName string `db:"schemaname"`
			}{}
			err := txApp.DB().Select("viewname", "definition", "schemaname").
				From("pg_views").
				Where(dbx.NewExp("schemaname = current_schema")).
				All(&views)
			if err != nil {
				return err
			}
			for _, view := range views {
				_, err = txApp.DB().NewQuery("DROP VIEW IF EXISTS " + view.SchemaName + "." + view.Name + " CASCADE").Execute()
				if err != nil {
					return err
				}
			}
			...
			if !isOldMultiple && isNewMultiple {
				...
				copyQuery = txApp.DB().NewQuery(fmt.Sprintf(
					`UPDATE {{%s}} SET [[%s]] = (
					    CASE
					        WHEN COALESCE(CAST([[%s]] AS TEXT), '') = ''
					        THEN '[]'::jsonb
					        ELSE (
					            CASE
					                WHEN pg_typeof([[%s]]) IN ('json'::regtype, 'jsonb'::regtype) AND jsonb_typeof(CAST([[%s]] AS jsonb)) = 'array'
					                THEN CAST([[%s]] AS jsonb)
					                ELSE jsonb_build_array(to_jsonb(CAST([[%s]] AS TEXT)))
					            END
					        )
					    END
					)`,
					    ...
					    ELSE (
					        CASE
					            WHEN jsonb_typeof([[%s]]::jsonb) = 'array'
					                THEN COALESCE([[%s]] ->> (jsonb_array_length([[%s]]) - 1), '')
					            ELSE [[%s]]::text
					        END
					    )
					END
			...
			// restore views
			for _, view := range views {
				viewQuery := fmt.Sprintf("CREATE OR REPLACE VIEW {{%s}}.{{%s}} AS %s", view.SchemaName, view.Name, view.Definition)
				_, err = txApp.DB().NewQuery(viewQuery).
					Execute()
				if err != nil {
					return err
				}
