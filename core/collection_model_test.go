...
func TestCollectionSaveViewWrapping(t *testing.T) {
	...
	scenarios := []struct {
		name         string
		query        string
		shouldBeCast bool // Whether the id should be cast to text
	}{
		{"no wrapping - text field", "select text as id, bool from demo1", false},
		{"no wrapping - id field", "select id, bool from demo1", false},
		{"no wrapping - relation field", "select rel_one as id, bool from demo1", false},
		{"no wrapping - email field", "select email as id, bool from demo1", false},
		{"no wrapping - datetime field", "select datetime as id, bool from demo1", false},
		{"no wrapping - url field", "select url as id, bool from demo1", false},
		{"wrapping - select field", "select select_many as id, bool from demo1", true},
		{"wrapping - bool field", "select bool as id, text as txt, url from demo1", true},
		{"wrapping - bool field (different order)", "select text as txt, url, bool as id from demo1", true},
		{"wrapping - json field", "select json as id, text, url from demo1", true},
		{"wrapping - numeric id", "select 1 as id", true},
		{"wrapping - expression", "select ('test') as id", true},
		{"wrapping - cast as text", "select cast('test' as text) as id", true},
	}
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			...
			// Verify SQL has expected wrapping pattern
			var viewDef string
			err = app.DB().NewQuery("SELECT definition FROM pg_views WHERE viewname={:name}").
			...
			// Check for presence of CAST operation
			hasCast := strings.Contains(viewDef, "::text") && strings.Contains(viewDef, "AS id")

			if s.shouldBeCast != hasCast {
				if s.shouldBeCast {
					t.Fatalf("Expected CAST to text, but none found in:\n%s", viewDef)
				} else {
					t.Fatalf("Found unexpected CAST in:\n%s", viewDef)
				}
			}

			// Quick functional test - verify the view works
			var count int
			...
			if err != nil {
				t.Fatalf("Failed to query the view: %v", err)
			}
		})
	}
}
