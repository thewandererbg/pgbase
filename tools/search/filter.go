...
func buildResolversExpr(
	...
	case fexpr.SignLike, fexpr.SignAnyLike:
		...
		if len(right.Params) == 0 {
			expr = dbx.NewExp(fmt.Sprintf("%s::text LIKE ('%%' || %s || '%%') ESCAPE '\\'", left.Identifier, right.Identifier), left.Params)
		} else {
			expr = dbx.NewExp(fmt.Sprintf("%s::text LIKE %s ESCAPE '\\'", left.Identifier, right.Identifier), mergeParams(left.Params, wrapLikeParams(right.Params)))
		}
	case fexpr.SignNlike, fexpr.SignAnyNlike:
		...
		if len(right.Params) == 0 {
			expr = dbx.NewExp(fmt.Sprintf("%s::text NOT LIKE ('%%' || %s || '%%') ESCAPE '\\'", left.Identifier, right.Identifier), left.Params)
		} else {
			expr = dbx.NewExp(fmt.Sprintf("%s::text NOT LIKE %s ESCAPE '\\'", left.Identifier, right.Identifier), mergeParams(left.Params, wrapLikeParams(right.Params)))
		}
...
func resolveEqualExpr(equal bool, left, right *ResolverResult) dbx.Expression {
	...
	nullExpr := "IS NULL"
	if !equal {
		// always use `IS DISTINCT FROM` instead of `!=` because direct non-equal comparisons
