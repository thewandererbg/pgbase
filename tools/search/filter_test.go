...
func TestFilterDataBuildExpr(t *testing.T) {
	...
	scenarios := []struct {
		...
		},
		{
			...
			"test1 ~ test2",
			false,
			"[[test1]]::text LIKE ('%' || [[test2]] || '%') ESCAPE '\\'",
		},
		{
			"like with right column operand",
			"'lorem' ~ test1",
			false,
			"{:TEST}::text LIKE ('%' || [[test1]] || '%') ESCAPE '\\'",
		},
		{
		...
		},
		{
			...
			"test1 !~ test2",
			false,
			"[[test1]]::text NOT LIKE ('%' || [[test2]] || '%') ESCAPE '\\'",
		},
		{
			"not like with right column operand",
			"'lorem' !~ test1",
			false,
			"{:TEST}::text NOT LIKE ('%' || [[test1]] || '%') ESCAPE '\\'",
		},
		{
			"like with left column operand and text as right operand",
			"test1 !~ 'lorem'",
		...
		},
		{
			...
			false,
			"(pb_json_extract([[test5]], '$.a') IS NOT DISTINCT FROM pb_json_extract([[test5]], '$.b') OR pb_json_extract([[test5]], '$.c') IS DISTINCT FROM pb_json_extract([[test5]], '$.d'))",
		},
		{
			"macros",
			`
		...
		},
		{
			...
			false,
			"(([[test1]] > {:TEST} OR [[test2]] IS DISTINCT FROM {:TEST}) AND [[test3]]::text LIKE {:TEST} ESCAPE '\\' AND ([[test4_sub]] = '' OR [[test4_sub]] IS NULL))",
		},
		{
		...
		},
		{
			...
			false,
			"((COALESCE([[test1]], '') = COALESCE([[test2]], '') OR COALESCE([[test2]], '') IS DISTINCT FROM COALESCE([[test3]], '')) AND ([[test2]]::text LIKE {:TEST} ESCAPE '\\' OR [[test2]]::text NOT LIKE {:TEST} ESCAPE '\\') AND {:TEST}::text LIKE ('%' || [[test1]] || '%') ESCAPE '\\' AND {:TEST}::text NOT LIKE ('%' || [[test2]] || '%') ESCAPE '\\' AND [[test3]] > {:TEST} AND [[test3]] >= {:TEST} AND [[test3]] <= {:TEST} AND {:TEST} < {:TEST})",
		},
		{
...
func TestFilterDataBuildExprWithParams(t *testing.T) {
	...
	expectedQuery := `SELECT * WHERE ([[test1]] = true OR [[test2]] = false OR [[test3a]] = 123.456 OR [[test3b]] = 123.456 OR ([[test4]] = '' OR [[test4]] IS NULL) OR [[test5]] = '""' OR [[test6]] = 'simple' OR [[test7]] = '''single_quotes''' OR [[test8]] = '"double_quotes"' OR [[test9]] = 'escape\\"quote' OR [[test10]] = '2023-01-01 00:00:00 +0000 UTC' OR [[test11]] = '["a","b","\\"quote"]' OR [[test12]] = '{"a":123,"b":"quote\\""}')`
